...
4. Git-commit the `data/ad-history.jsonl` append after executing decisions. The commit message format is contractual: `ops: YYYY-MM-DD N decisions, $X spend, Y leads, Z hot`.
5. If anything in the loop errors (MCP auth, sheet unreachable, rate limit), halt execution, escalate all pending decisions to `requires_approval: true`, and include the error at the top of the briefing.
