...
**Form rules (enforced by `form-cro` skill):**
- 3 fields only. Name + phone + interest. Email becomes optional progressive disclosure on thank-you page.
- Phone format hint "WhatsApp preferred."
- No budget or timeline dropdowns in the first form — research shows each additional field costs 4–8% conversion. Qualify in the follow-up conversation.

**Anti-slop (from CLAUDE.md):** No "nestled in", "boasts", "stunning", "paradise awaits", em-dashes, filler adjectives. First-person, specific, concrete. One CTA per page section.

**Pull from `data/property.json`:** hero copy, pricing mentions, inventory counts, amenities, timeline, media paths, contact info. Never hardcode.

**Measurement:** Meta Pixel `Lead` event fires on form submit with `event_id` for CAPI dedup. UTM params captured into form payload.
