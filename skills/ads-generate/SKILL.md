...
1. **Prefer real photography from `data/property.json` media fields** (drone footage stills, photos, lot map). Real-estate creative converts better with authentic imagery than AI renders — AI should fill gaps only.
2. Only fall back to AI generation when no real asset covers the required angle/format and the `nanobanana-mcp` is configured. If it is not configured, return a brief of what assets to commission from the owner instead of failing.

**Context to read before generating (even for AI):**
- `property-context` skill output — for location vibe, project character, target segments.
- `data/scoring-model.json` → `audience_segments` — generate one variant per relevant segment.

**Format matrix for Meta (required sizes):**
- Feed: 1080×1080 (square) or 1080×1350 (4:5, preferred).
- Stories/Reels: 1080×1920 (9:16), hook visible in top 1/3.
- Carousel: 1080×1080 per card, 3–5 cards, narrative arc (problem → project → proof → offer → CTA).
- Retargeting: variation of feed creative with overlay copy referencing site visit.
...
**Anti-slop in every overlay:** no "nestled," "boasts," "stunning," no em-dashes, no "discover your dream" — direct, specific, concrete copy only (CLAUDE.md project rules).

**Output:** list of `{segment, format, size, visual_brief, overlay_copy, source: "real_photo" | "ai_generated", file_path_or_prompt}`.
