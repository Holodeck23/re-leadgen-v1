...
**Invocation context:** called by `reflective-ops` daily (and `creative-auditor` agent). You run inside a lead-gen system with 1 active advertiser account.

**Data sources to use, in order:**
1. `data/property.json` via `property-context` — for what "on-brand" creative should reference.
2. Meta Ads MCP (Pipeboard, `mcp__meta-ads__*`) — for live account metrics. Evaluate across 4 timeframes: today / yesterday / last_7d / lifetime. Never judge on single-day.
3. `data/kill-scale-rules.json` — use these thresholds, not the generic ones in the upstream 50-check list. The RE thresholds supersede.
4. `data/ad-history.jsonl` — context on prior decisions; don't repeat killed experiments.
5. `scripts/sheet-ops.py quality-by-adset` — lead quality per ad set. Weight any "scale this" verdict on whether leads actually qualify.

**Upstream references (read for depth if needed):** `ads/references/meta-audit.md`, `ads/references/benchmarks.md`, `ads/references/scoring-system.md` — these are in the submodule at `vendor/claude-ads/ads/references/`.

**Output shape:** A structured finding list `[{check_id, status: PASS|WARN|FAIL, metric, observed, threshold, recommended_action, requires_approval, confidence}]`. `reflective-ops` reads this to build the decision queue.

**Non-negotiable checks (fail loud):**
- Meta Pixel firing + CAPI active + EMQ ≥8 on `Lead` event.
- Creative similarity — flag if <5 genuinely distinct concepts active (Andromeda will suppress).
- Frequency per ad set — FAIL if >3.0 after $30 spend.
- Kill-rule violations from `kill-scale-rules.json` → status=FAIL, action=pause, auto-approved.
