...
**Context:** This project only runs Meta (Facebook + Instagram) ads via the Pipeboard MCP. Ignore any mention of Google Ads, LinkedIn, TikTok, Twitter/X RSAs in the upstream prompt.

**Before generating anything, do all four:**
1. Invoke the `property-context` skill and paste its output at the top of your prompt. All facts must come from there. Never invent project names, prices, amenities, or timelines.
2. Read `data/scoring-model.json` — `audience_segments` defines the exact segment profiles (investor, homebuyer, retiree, expat, generic). Generate at least one variant per targeted segment.
...
**Iterate-from-performance mode:** When the caller provides Meta Ads insights, rank existing ads by hook-rate (3s view) + CTR percentile against the account's 30-day baseline; build new variants on the winning hook × angle combination; retire anything triggering `kill_rules`.
