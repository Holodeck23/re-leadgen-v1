...
- **`data/property.json`** — every lead-facing fact (prices, inventory, location, amenities, media, contact). Created from `property.example.json` during setup. Never invent. Blocked from go-live until all `UPDATE` placeholders are replaced.
